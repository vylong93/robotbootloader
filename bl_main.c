...
uint8_t waitForRfPacket(uint32_t ui32TimeOut, uint8_t *pui8RfBuffer)
{
  ...
  while (ui32BootTime < ui32TimeOut)
  {
    ...
    {
      // get interrupt flag::RF24_getIrqFlag(RF24_IRQ_RX)
      clearRfCSN();
      ui8RfStatus = SPI_sendAndGetData(RF24_COMMAND_NOP);
      setRfCSN();
      if (ui8RfStatus & RF24_IRQ_RX)
      {
        // get pay load width::RF24_RX_getPayloadWidth()
        clearRfCSN();
        ui8RfStatus = SPI_sendAndGetData(RF24_COMMAND_R_RX_PL_WID);
        ui8RxLength = SPI_sendAndGetData(RF24_COMMAND_NOP);
        setRfCSN();

        // get pay load data::RF24_RX_getPayloadData(ui8RxLength, pui8RfBuffer)
        clearRfCSN();
        ui8RfStatus = SPI_sendAndGetData(RF24_COMMAND_R_RX_PAYLOAD);
        for (i = 0; i < ui8RxLength; i++)
        {
                *(pui8RfBuffer + i) = SPI_sendAndGetData(RF24_COMMAND_NOP);
        }
        setRfCSN();

        // clear rf interrupt flag::RF24_clearIrqFlag(RF24_IRQ_RX)
        clearRfCSN();
        ui8RfStatus = SPI_sendAndGetData(RF24_REG_STATUS | RF24_COMMAND_W_REGISTER);
        ui8RfStatus &= 0x0F;
        SPI_sendAndGetData(RF24_IRQ_RX | ui8RfStatus);
        setRfCSN();
      ...
      {
        // clear rf interrupt flag::RF24_clearIrqFlag(RF24_IRQ_MASK)
        clearRfCSN();
        ui8RfStatus = SPI_sendAndGetData(RF24_REG_STATUS | RF24_COMMAND_W_REGISTER);
        ui8RfStatus &= 0x0F;
        SPI_sendAndGetData(RF24_IRQ_MASK | ui8RfStatus);
        setRfCSN();
      }
    }
    ui32BootTime++;
...
uint32_t CheckForceUpdate(void)
{
  ...
  uint8_t pui8RfBuffer[BSL_START_PACKET_LENGTH];
  ...
  if (ui8RxLength == BSL_START_PACKET_LENGTH)
  {
    ...
    ui8CheckSum += pui8RfBuffer[0] + pui8RfBuffer[1] + pui8RfBuffer[2] + pui8RfBuffer[3]
                    + pui8RfBuffer[4];
    ...
    {
      g_ui32TransferSize = convertByteToUINT32(
                      pui8RfBuffer + TRANSFER_SIZE_OFFSET);
...
//  HWREG(NVIC_VTABLE) = VTABLE_START_ADDRESS;
//
//  __asm(
//                  "    movw    r0, #0x2000\n"
//                  "    ldr     sp, [r0]\n"
...
void Updater(void)
{
  ...
  if (BL_FLASH_ERROR_FN_HOOK())
  {
    ExitBootloader();   // eBootStatus = BL_FLASH_ACCESS_FAILED;
  }
  ...
  while (ui32ExpectedAddress < ui32ProgramEndAddress)
  {
    ...
    //ui8RxLength = waitForRfPacket((g_1msCycles << 1) / 5, pui8RfBuffer);
    ...
    if (ui8RxLength != 0)
    {
      ...
      if (ui32ReceivedAddress == ui32ExpectedAddress)
      {
        ...
        if ((pui8RfBuffer[BSL_PACKET_BYTECOUNT_OFFSET] % 4) != 0)
        {
          eBootStatus = BL_INVALID_DATA_LENGTH;
		  NACK();
          //RF24_TX_jamming(50000);
          continue;
        }
        ...
        // Write this block of data to the flash
        BL_FLASH_PROGRAM_FN_HOOK(ui32ReceivedAddress,
                        (uint8_t * )(pui8RfBuffer + BSL_PACKET_DATA_OFFSET), pui8RfBuffer[BSL_PACKET_BYTECOUNT_OFFSET]);
        ...
        ui32ExpectedAddress += pui8RfBuffer[BSL_PACKET_BYTECOUNT_OFFSET];
